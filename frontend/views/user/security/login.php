    ...
    <div class="container">
        <div class="about_inner">
            <div class="about_item">
                <form action="<?= Url::to(["/user/security/login"]) ?>" method="post"
                      class="ask_form aos-init aos-animate"
                      data-aos="fade-up">
                    <input type="hidden" name="<?= Yii::$app->request->csrfParam; ?>"
                           value="<?= Yii::$app->request->getCsrfToken(); ?>"/>
                    <?= Html::activeInput("text", $model, 'login', ["placeholder" => Yii::t("user", "Login"), "class" => "input"]) ?>
                    <?= Html::activeInput("password", $model, 'password', ["placeholder" => Yii::t("user", "Password"), "class" => "input"]) ?>
                    <?= Html::submitButton(Yii::t("user", "Sign in"), ["class" => "btn"]) ?>
                </form>
            </div>
        </div>
    </div>
